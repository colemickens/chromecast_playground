...
(function() {
  ...
  function parseQualityLevel(xml) {
    var result = {};
  ...
  function parseStreamIndex(xml) {
    ...
    result.QualityLevels = parseChildElements(
        'QualityLevel', parseQualityLevel, xml);
  ...
  function createAdaptationSets(streamIndexes, contentProtection) {
    ...
    // TODO: Only use the last audio and video Streams
    var validStreams = [];
    var audio = false;
    var video = false;
  ...
  function parseManifest(xml) {
    ...
    //TODO (afontan) let's simply map the SS manifest into the mpd manifest
    //for now. mpd-style attributes:
    // mediaPresentationDuration
    result.duration = result.Duration / result.TimeScale;

    //adaptationsSets
    if (result.Protection) {
      // TODO get this from the PR Header object
