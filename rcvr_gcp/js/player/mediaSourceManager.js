...
(function() {
  ...
  // TODO: (afontan) Review the definition if isActive.
  // What happens if readyState is not open
  MediaSourceManager.prototype.areAllActive = function() {
    ...
    if (!active && this.mediaSrc_.readyState === 'open') {
      // TODO (afontan) Handle end of stream
      log('Ending stream');
  ...
  MediaSourceManager.prototype.onSourceOpen =
    ...
    for (i = 0; i < this.manifest_.adaptationSets.length; i++) {
      ...
      // TODO (afontan) verify that rerunning this multiple times is correct
  ...
  //TODO (afontan) consolidate with attach when mediaSrc_ is not used externally
