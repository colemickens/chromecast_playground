...
(function() {
  ...
  // TODO (afontan) Only SEEK_RESUME should be here
  var BUFFERING = {
    ENOUGH: 20,
    SAFE: 5,
    LOW: 2,
  ...
  SourceBufferManager.prototype.reset = function(reason) {
    ...
    //TODO (afontan) Figure out if this is needed and why
  ...
  SourceBufferManager.prototype.identifyNextFragment = function(currentTime) {
    ...
    if (this.segIndex == null) {
      ...
      //TODO (afontan): Do we still need this?
  ...
  SourceBufferManager.prototype.appendData = function(data) {
    ...
    // TODO: (afontan) Verification of current buffer.
    ...
    // TODO (afontan) SEEK_ISSUE2 b228903
    ...
    // TODO (afontan): Enable auditing
    dlog(4, 'Ranges in appendData2: ' + this.getRangesString());

    /* TODO (afontan) Add verification
  if (xhr.expected_time != null) {
    ...
    // TODO: (afontan) Verification of current buffers.
    dlog(4, 'Expected Time: ' + xhr.expected_time + ' Ranges in onXHRLoad3: ' +
      this.getRangesString(buf));
    var range = this.findRangeForPlaybackTime(buf, xhr.expected_time);
    if (range == null ||
      ...
      if (range == null) {
        dlog(4, 'Reason: range is null');
      } else {
        dlog(4, 'Reason: expected time ' + xhr.expected_time + ' not in [' +
              range.start + ', ' + range.end + ']');
      }
    }
  }
  else {
    dlog(3, 'expected_time is null');
  }
  */
  };
